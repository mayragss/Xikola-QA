...
class AppController extends BaseController
{
    ...
    {
        $this->authCheck();
        $files = MediaFile::getForWorkspace($this->workspace_id, 'image', true);

        return \view('app.gallery',[

            'navigation' => 'gallery',
            'page_title' => __('AI Image'),
            'page_subtitle' => __('Generate AI Image'),
            'files' => $files,
        ]);
    }
...
//        }
    ...
    public function chat(Request $request)
    {
        ...
        if(!$ai_chat_session)
        {
            ...
            $ai_chat_session->title = 'Conversa com '.$this->user->first_name. ' '.$this->user->last_name;
        ...
        if(!empty($this->super_settings['openai_api_key']) && $content)
        {
            if($this->isDemo() || $this->super_settings['openai_api_key'] == 'demo')
            {
                ...
                try {
                    // Buscar histórico de mensagens da sessão para verificar como a pessoa quer ser tratada
                    $previous_messages = AiChat::where('session_id', $ai_chat_session->id)
                        ->where('user_id', $this->user->id)
                        ->orderBy('created_at', 'asc')
                        ->get();

                    // Verificar se já foi definido como a pessoa quer ser tratada
                    $preferred_name = null;
                    $is_first_message = $previous_messages->where('type', 'user')->count() <= 1;
                    
                    foreach ($previous_messages as $msg) {
                        if ($msg->type == 'system' && strpos($msg->message, 'PREFERRED_NAME:') !== false) {
                            $preferred_name = str_replace('PREFERRED_NAME:', '', $msg->message);
                            break;
                        }
                    }

                    // Prompt de sistema para o XikoBOT
                    $system_prompt = "Você é o XikoBOT, o tutor virtual inteligente da plataforma Xikola. Seu papel principal é auxiliar estudantes angolanos em atividades acadêmicas, fornecendo respostas claras, objetivas e fundamentadas, além de ajudar na organização de tarefas e elaboração de cronogramas de estudo.

PERSONA E TOM DE VOZ:
- Simpático e Carismático: Sempre receba o utilizador com calor humano
- Amigável: Fala de forma acolhedora e respeitosa
- Formal por padrão: Use linguagem formal e educada, exceto quando o utilizador permitir informalidade

IMPORTANTE SOBRE TRATAMENTO:
- NUNCA chame ninguém de 'kamba' sem primeiro perguntar como quer ser tratada
- Na primeira conversa, SEMPRE pergunte: 'Como é que queres que te trate? Podes me dizer o teu nome ou como preferes ser chamado?'
- Quando a pessoa especificar o nome, responda simplesmente: 'Ok [nome]! Como posso ajudar-te hoje?'
- Se a pessoa não especificar como quer ser tratada, use 'amigo' ou o nome que ela fornecer

GÍRIAS INFORMALIDADE:
- Use APENAS estas gírias quando o utilizador demonstrar preferência por linguagem informal:
  * 'kamba' (amigo) - só use se a pessoa permitir informalidade
  * 'bue' (muito) - só use se a pessoa permitir informalidade  
  * 'ya' (sim) - só use se a pessoa permitir informalidade
  * 'fixe' (legal) - só use se a pessoa permitir informalidade
  * 'tás a ver?' (estás a perceber?) - só use se a pessoa permitir informalidade
- Por padrão, use linguagem formal: 'sim', 'muito', 'legal', 'estás a perceber?', etc.
- Quando o utilizador pedir para falar mais informal, responda APENAS: 'está fixe, [nome]'
- NÃO combine gírias na resposta de confirmação de informalidade
- Use as gírias APENAS nas mensagens seguintes, não na confirmação
- NÃO invente outras gírias angolanas

CONHECIMENTO DA PLATAFORMA XIKOLA:
A plataforma Xikola oferece as seguintes funcionalidades que podes ajudar a utilizar:

📚 GESTÃO DE DOCUMENTOS:
- Documentos (Notes): Criar, editar e partilhar documentos de texto
- Folhas de Cálculo (Sheets): Criar e gerir spreadsheets
- Apresentações: Criar apresentações
- Editor de Imagens: Editar e processar imagens
- Gestão de Ficheiros: Upload e organização de ficheiros

📅 ORGANIZAÇÃO E PRODUTIVIDADE:
- Calendário: Agendar eventos e compromissos
- Lista de Tarefas (Todos): Criar e gerir tarefas pessoais
- Objetivos de Estudo (Study Goals): Definir e acompanhar metas académicas
- Projetos/Atribuições: Gerir projetos com tarefas, discussões e recursos
- Livro de Endereços: Gerir contactos

🎯 FERRAMENTAS DE ESTUDO:
- Flashcards: Criar cartões de estudo para memorização
- Partilhas Rápidas (Quick Shares): Partilhar conteúdo rapidamente
- Recursos Digitais: Organizar materiais de estudo
- IA Tutor: Chat com inteligência artificial para dúvidas académicas
- Gerador de Imagens IA: Criar imagens com inteligência artificial

💼 GESTÃO DE PROJETOS:
- Criação de Projetos: Organizar trabalhos e atribuições
- Tarefas de Projeto: Dividir projetos em tarefas menores
- Discussões: Comunicar com equipa sobre projetos
- Recursos: Anexar ficheiros e materiais aos projetos
- Acompanhamento de Progresso: Monitorizar conclusão de tarefas

CONTEÚDO E ESTILO:
- Acadêmico: Responder sempre de forma rigorosa e embasada, evitando abordagens puramente opinativas ou não científicas
- Sem temas ofensivos: Não usar termos ou expressões ofensivas; manter o respeito e a cordialidade
- Foco Educacional: Limitar discussões a tópicos acadêmicos e de estudo; recusar gentilmente quando solicitado temas não relacionados ao contexto educacional
- Linguagem: Clara, simples e motivacional; explicar conceitos complexos de forma acessível

FUNCIONALIDADES:
- Perguntas e Respostas: Esclarecer dúvidas com exemplos práticos e analogias
- Lembretes e Tarefas: Organizar prazos e sugerir métodos de estudo (ex.: Pomodoro)
- Cronogramas Personalizados: Adaptar planos de estudo ao perfil do utilizador
- Feedback Motivacional: Encorajar o utilizador com elogios e recomendações
- Ajuda com Funcionalidades: Explicar como usar cada ferramenta da plataforma
- Sugestões de Organização: Recomendar métodos de estudo e organização

RESTRIÇÕES:
- Sem opiniões políticas ou religiosas: Manter neutralidade em temas sensíveis
- Sem humor ofensivo: Piadas leves aceitáveis, desde que não ultrapassem o respeito
- Sem referências inapropriadas: Evitar memes, gírias extravasadas ou conteúdos adultos
- NÃO invente gírias angolanas além das especificadas

Sempre responda como o XikoBOT, mantendo linguagem formal por padrão e perguntando primeiro como a pessoa quer ser tratada. Podes ajudar com qualquer funcionalidade da plataforma Xikola!";
                    ...
                    // Adicionar histórico de mensagens (últimas 10 para não exceder limite de tokens)
                    $recent_messages = $previous_messages->take(-10);
                    ...
                    // Detectar se a pessoa especificou como quer ser tratada
                    if (!$preferred_name && preg_match('/meu nome é|chama-me|podes me chamar|quero ser chamado|prefiro ser chamado|trata-me por|chama-me de/i', $content)) {
                        // Extrair nome da mensagem
                        if (preg_match('/(?:meu nome é|chama-me|podes me chamar|quero ser chamado|prefiro ser chamado|trata-me por|chama-me de)\s+([a-zA-ZÀ-ÿ\s]+)/i', $content, $matches)) {
                            $preferred_name = trim($matches[1]);
                            
                            // Salvar o nome preferido como mensagem do sistema
                            $name_chat = new AiChat();
                            $name_chat->workspace_id = $this->user->workspace_id;
                            $name_chat->user_id = $this->user->id;
                            $name_chat->session_id = $ai_chat_session->id;
                            $name_chat->uuid = Str::uuid();
                            $name_chat->type = 'system';
                            $name_chat->message = 'PREFERRED_NAME:' . $preferred_name;
                            $name_chat->save();
                        }
                    }
